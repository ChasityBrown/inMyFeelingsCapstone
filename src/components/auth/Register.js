...
export const Register = (props) => {
    ...
    const existingUserCheck = () => {
        return fetch(`http://localhost:8098/users?email=${user.email}`)
            .then(res => res.json())
    ...
    const handleRegister = (e) => {
        ...
        existingUserCheck()
            .then((userExists) => {
                if (!userExists) {
                    fetch("http://localhost:8098/users", {
                        method: "POST",
                        headers: {
                        ...
                        .then(createdUser => {
                            if (createdUser.hasOwnProperty("id")) {
                                localStorage.setItem("feelings_user", createdUser.id)
                                history.push("/")
                            }
                        })
                }
    ...
    return (
        <main style={{ textAlign: "center" }}>
            ...
            <form className="form--login" onSubmit={handleRegister}>
                <h2 className="h3 mb-3 font-weight-normal">Register for "All in My Feelings"</h2>
                <fieldset>
